...
The workspace contains three published crates plus two non-published packages:
...
| `rados-cls` | Client-side encodings for Ceph object classes (`version`, `refcount`, `user`, `queue`, `rgw`, `rgw_gc`, `lock`, `two_pc_queue`), one module per class behind a feature of the same name. |
